...
contract Market is IERC721Receiver {
    ...
    function buy(uint256 _tokenId, uint256 _price) external {
        // 此处编写业务逻辑
        require(isListed(_tokenId), "the token is not for sale!");
        require(_price >= orderOfId[_tokenId].price, "your price is too low!");
    ...
    function cancelOrder(uint256 _tokenId) external {
        // 此处编写业务逻辑
        require(isListed(_tokenId), "the token is not list!");
        address seller = orderOfId[_tokenId].seller;
        require (msg.sender == seller, "you are not the token owner!");
        IERC721(erc721).safeTransferFrom(address(this), seller, _tokenId);
        removeListing(_tokenId);
    ...
    function placeOrder(
        ...
        // 此处编写业务逻辑
        require(!isListed(_tokenId), "the token has already list!");
        ...
        orders.push(order);
        idToOrderIndex[_tokenId] = orders.length;
        emit NewOrder(_seller, _tokenId, _price);
    }

    function removeListing(uint256 _tokenId) internal {
        // 此处编写业务逻辑
        orders[idToOrderIndex[_tokenId] - 1] = orders[orders.length - 1];
